    ...
    <div className="cm-in space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {state.roommates.map((r) => {
          const canEditStatus = isAdmin || r.id === me?.id;
          return (
            <Card key={r.id} className="relative p-4">
              ...
              <div className="mt-3 flex items-center gap-1.5">
                <Btn
                  className={cn("flex-1", !canEditStatus && "opacity-40 cursor-not-allowed")}
                  variant={r.status === "active" ? "solid" : "ghost"}
                  disabled={!canEditStatus}
                  title={!canEditStatus ? "Only this roommate or Admin can update status" : undefined}
                ...
                <button
                  ...
                  className={cn(
                    "px-2.5 py-2 text-xs font-semibold rounded-lg transition-all",
                    r.status === "away" ? "text-sky-400 font-bold bg-sky-400/10 border border-sky-400/30" : "text-muted hover:text-ink",
                    !canEditStatus && "opacity-40 cursor-not-allowed hover:text-muted",
                  )}
                >
                  Away
                </button>
